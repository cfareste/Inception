...
##### Docker volume:
As we already stated, data stored inside a Docker container is lost when the container is deleted. To prevent it, Docker brings to the table two options: volumes and bind mounts. Both are persistent data stores, with some differences between them:
- Docker volumes: Volumes are created and managed entirely by Docker. Volumes are stored within the directory of the Docker host (server-side of Docker). Volumes are preferred on production, as they are more portable (work on both Linux and Windows), are easier to backup and migrate, they are manageable with the Docker client... A volume is mounted on a concrete container directory. For example, if you want to save the data inside the `/var/lib/mariadb/database` container's directory, then you would create a volume that is stored in the Docker host and would be mount on top of that directory.
- Bind mounts: Bind mounts are Linux's bind mounts. A Linux's bind mount is a directory structure (tree) that is replicated and synced inside another directory, meaning all the files and directories inside the mounted directory will be copied into the other one, and every change in one side is immediately reflected on the other side. When you use a bind mount with a container, you mount a host machine directory on a container directory. This lets you share and persist files of a container inside the host machine, useful in development.

# CHANGE THE MOUNTING DEFINITIONS!
